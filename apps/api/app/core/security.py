...
def _verify_jwt_via_supabase_auth(token: str) -> TokenPayload:
    ...
    if not anon_key or anon_key == "your-anon-key":
        raise HTTPException(
            ...
            detail="La API no puede validar sesiones: configuración de autenticación incompleta",
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="No se pudo contactar al servicio de autenticación para validar el token",
        ) from exc
    ...
    if not user_id:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Respuesta del servicio de autenticación sin identificador de usuario",
        )
