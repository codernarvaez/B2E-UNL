...
## 2025-07-06 - [Fix unescaped dynamic variables in .innerHTML assignments]
**Vulnerability:** In `apps/web/src/islands/AdminPanel.ts` and `apps/web/src/islands/CompanyChallengeForm.ts`, some dynamic variables (like object IDs or fallback strings) were interpolated directly into HTML strings within `.innerHTML` assignments without being passed through the `esc()` or `escapeHtml()` utility functions. This could lead to a Cross-Site Scripting (XSS) vulnerability.
**Learning:** Even internal properties or IDs not usually provided directly by users can be manipulated in certain attack vectors, and they can break out of attributes. It is crucial to escape absolutely all dynamic interpolations inside `.innerHTML` strings, no matter what they seem to hold.
**Prevention:** Always wrap all dynamic variables (including HTML attributes and fallback strings like `a ?? b`) with the `esc()` or `escapeHtml()` utility functions before injecting them via `.innerHTML` assignments.
