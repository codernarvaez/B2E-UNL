...
## 2024-07-28 - [Fix Information Disclosure in JWT Verification Fallback]
**Vulnerability:** The API returned specific backend infrastructure details ("Supabase Auth", `.env` variable names, and internal HTTP status codes) to unauthenticated users when JWT verification via the fallback service failed. The frontend error handler then relayed these exact strings in error alerts, presenting a clear information leakage vulnerability.
**Learning:** Exception handling paths, particularly those related to authentication and upstream services, must be carefully reviewed to ensure they fail securely and emit generic error messages to the client. This includes checking logs generated on startup.
**Prevention:** Avoid embedding internal error states, `.env` variables, or dependency names (like Supabase Auth, database errors, etc.) in user-facing HTTP response details, frontend error alerts, and stdout server logs. Use generic messages like "Servicio no disponible" or "Credenciales inválidas" to avoid mapping attack surfaces.
