...
## 2025-07-13 - [Preventing XSS in Astro Islands]
**Vulnerability:** Several variables (like IDs, object lookups, and fallback strings) were directly interpolated without escaping into HTML strings used for `.innerHTML` assignment within client-side TypeScript islands (e.g., `AdminPanel.ts`, `CompanyChallengeForm.ts`).
**Learning:** Even variables that seem safe or are nested (like object lookups `roleLabels[u.role]`, IDs `c.id`, or function results `badgeClass(u.approval_status)`) can be vectors for XSS if user-controlled data influences them. In Astro islands where manual DOM manipulation is used, every single dynamic value must be explicitly escaped.
**Prevention:** Always systematically wrap every interpolated variable inside HTML template literals with a robust escaping function (like `esc()` or `escapeHtml()`) before injecting it via `.innerHTML`. Consider using safer DOM APIs like `.textContent` when HTML formatting is not required.
